...
export default class CreatorHelper extends WorkspaceHelper {
  constructor() {
    super();
  }
  /* common */
  ...
  getError(msg) {
    const errors = {
      ...
      notCategory: "Musisz wybrać kategorię",
      ...
      linkError: "Podany link jest niepoprawny lub coś poszło nie tak",
      emptyFile: "Musisz dodać jakiś materiał",
      emptyTitle: "Musisz dodać tytuł",
      emptyTitleGraphic: "Musisz dodać gówny lub dolny tytul",
      emptyDescription: "Musisz dodać opis",
      notConfirm: "Musisz potwierdzić regulamin",
      titleTooLong: "Tytuł może zawierać maksimum 250 znaków",
      descriptionTooLongGraphic:
        "Opis dla grafiki może zawierać maksimum 750 znaków",
      descriptionTooLongPost:
        "Opis dla postu może zawierać maksimum 10 000 znaków",
      captcha: "Nie jesteś człowiekiem?",
      youtubeError: "Link jest nieprawidłowy lub coś poszło nie tak",
    };
    return errors[msg];
  }
  ...
  switcherMain ()  {
    const tabs = document.querySelectorAll(".creator-switcher__menu-tab");
    const contents = document.querySelectorAll(".creator__content");
    tabs.forEach((tab) => {
      tab.addEventListener("click", (e) => {
        const tab = e.currentTarget;
        const name = tab.dataset.name;
        clearTabs(tabs, "creator-switcher__menu-tab");
        tab.classList.add("creator-switcher__menu-tab--active");
        clearContents(contents);
        document
          .querySelector(`.creator__content--${name}`)
          .classList.remove("display-none");
          document.querySelector(`input[name='postType']`).value = name;
      });
    });
